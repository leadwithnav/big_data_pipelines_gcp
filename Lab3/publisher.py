#!/usr/bin/env python
"""
Lab 3: IoT Telemetry CSV-Based Publisher
This script reads simulated sensor data from a CSV file and publishes the records 
indefinitely in a loop to a Google Cloud Pub/Sub topic, injecting the latest 
UTC timestamp into each message.
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Simulate real-time IoT Pub/Sub Telemetry from CSV")
    parser.add_argument(
        "--project",
        required=True,
        help="GCP Project ID"
    ...
    parser.add_argument(
        "--topic",
        default="beam-input",
        help="Pub/Sub Topic name (default: beam-input)"
    )
    parser.add_argument(
        "--csv",
